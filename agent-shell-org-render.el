...
(defun agent-shell-org-render--source-blocks ()
  ...
  (let ((blocks '())
        (case-fold-search nil)
        (re (rx bol (zero-or-more whitespace) (group "```") (zero-or-more whitespace)
    ...
    (save-excursion
      ...
      (while (re-search-forward re nil t)
        (when-let ((begin (match-beginning 0))
                   (end (match-end 0)))
          (push (list 'start (cons (match-beginning 1) (match-end 1))
                      'end (cons (match-beginning 4) (match-end 4))
                      'language (when (and (match-beginning 2) (match-end 2))
